	...
	<view class="contaier" :style="{ minHeight: getHeight + 'px' }">
		...
		<view class="padding">
			<view class="mainBox radius shadow-warp bg-white margin-top relative">
				<view class="userHeard cu-avatar lg round" v-bind:class="{ userHeardBg: end }">{{number}}</view>
				<view v-show="hasData">
					<view class="animation-reverse shadow" :class="animation" data-class="slide-rights"
						...
						<view class="mentalList" v-for="(item, index) in DataList[num].itemList"
							v-bind:class="{ errorChoose: index == mentalListActive }" @tap="Toggle(index)">
...
<script>
	export default {
		...
		methods: {
			...
			countDownTime() {
				console.log("倒计时：" + this.number + "s")
				this.number = 10;
				const that = this;
				if (that.number <= 10) {
					var sid = setInterval(function() {
						that.number = that.number - 1;
						console.log("倒计时==" + that.number)
						if (that.number === 0) {
							clearInterval(sid);
							// TODO 超时答题
							if (!this.end) {
...
<style lang="scss" scoped>
	...
	.userHeardBg {
		background-image:url(http://cdn.zhoukaiwen.com/logo.png);
	}
	...
	.mentalListActive {
		...
		margin: 30rpx 0;
		font-weight: 600;
		background-image: url("@/static/answer/yes.png");
		/*background-image: require("@/static/answer/yes.png");*/
	...
	.errorChoose {
		...
		margin: 30rpx 0;
		font-weight: 600;
		background-image: url("@/static/answer/no.png");
		/*background-image: require("@/static/answer/yes.png");*/
		background-repeat: no-repeat;
		background-size: 38rpx;
		background-position: right 30rpx center;
